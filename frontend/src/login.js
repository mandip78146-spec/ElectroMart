...
export const Login = () => {
    ...
    const login = async (e) => {
        ...
        if (result.ok && res.statuscode === 1) {
            clearStoredToken()
            localStorage.setItem("data", res.jwtoken)
            const role = res.userType || res.data?.UserType || res.data?.usertype || "User";
            setutype(role)
            setid(res.data?._id || "")
            Swal.fire({
                icon: "success",
        ...
        Swal.fire({
            icon: "error",
            title: "Login Error",
            text: res.message || "Check Password and Mail is Correct"
        })
    }
