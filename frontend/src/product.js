...
export const Product = () => {
    ...
    const show2 = async () => {
        const result = await fetch(`http://localhost:8000/api/getbrand2/${id2}`, {
            method: "get"
        })
        if (result) {
            const res = await result.json()
                                    ...
                                    <div key={a._id}>
                                        <div className="card  w-100 my-auto mt-3   " style={{ height: "auto" }}>
                                            <div className="d-flex align-items-center justify-content-between px-4 py-2">
                                                <div>
                                                    <img style={{ height: "80px", width: "80px" }} src={getImageUrl(a.Img)} alt={a.ProductName || 'product'}></img></div>
                                                <div>
